...
class OfficeDashboardController extends OfficeBaseController
{
    public function index()
    {
        ...
        $pendingRequests = ServiceRequest::where('office_id', $office->id)->where('status', 'pending')->count();
        $completedRequests = ServiceRequest::where('office_id', $office->id)->where('status', 'completed')->count();
