        ...
        <div class="panel">
            ...
            <p>
                ...
                @if($serviceRequest->submitted_at)
                    {{ \Illuminate\Support\Carbon::parse($serviceRequest->submitted_at)->timezone(config('app.timezone'))->format('M d, Y h:i A') }}
                @else
                    Not submitted
                @endif
            </p>
                ...
                <div class="message">
                    ...
                    <small>
                        ...
                        @if($history->changed_at)
                            {{ \Illuminate\Support\Carbon::parse($history->changed_at)->timezone(config('app.timezone'))->format('M d, Y h:i A') }}
                        @else
                            No date
                        @endif
                    </small>
                            ...
                            <tr>
                                ...
                                <td>
                                    @if($document->uploaded_at)
                                        {{ \Illuminate\Support\Carbon::parse($document->uploaded_at)->timezone(config('app.timezone'))->format('M d, Y h:i A') }}
                                    @else
                                        No date
                                    @endif
                                </td>
                            ...
                            <tr>
                                ...
                                <td>
                                    @if($document->generated_at)
                                        {{ \Illuminate\Support\Carbon::parse($document->generated_at)->timezone(config('app.timezone'))->format('M d, Y h:i A') }}
                                    @else
                                        No date
                                    @endif
                                </td>
                    ...
                    <tbody>
                        @forelse($serviceRequest->payments as $payment)
                            <tr>
                                ...
                                <td>{{ $payment->provider ?? 'No provider' }}</td>

                                <td>{{ $payment->transaction_reference ?? 'No reference' }}</td>

                                <td>
                                    @if($payment->paid_at)
                                        {{ \Illuminate\Support\Carbon::parse($payment->paid_at)->timezone(config('app.timezone'))->format('M d, Y h:i A') }}
                                    @else
                                        Not paid yet
                                    @endif
                                </td>
                                ...
                                <td>
                                    @if($payment->payment_method === 'crypto' && $payment->status !== 'success')
                                        <a href="{{ route('citizen.crypto-payments.show', $payment->id) }}" class="button secondary payment-action-button">
                                            Pay Crypto
                                        </a>
                                    @else
                        ...
                        @empty
                            <tr>
                                <td colspan="7">No payments found for this request.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
